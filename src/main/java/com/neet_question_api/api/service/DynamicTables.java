...
public class DynamicTables {
    ...
    public List fetchRandomQuestionsFromTable(String tableName, String limit, String excludedFeild) {
        ...
        try {
            String query = String.format("SELECT id,question,option1,option2,option3,option4 FROM %s ORDER BY RAND() LIMIT %s", tableName.toLowerCase(), limit);
    ...
    public String fetchAnswerByIdFromTable(String tableName, int id) {
        try {
            ...
            String query = String.format("SELECT answere FROM %s WHERE id = :id", tableName.toLowerCase());
